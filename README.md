...
[//]: # (Image References)
[image1]: ./P5/Submit/examples/test1.jpg "Test image"
[image2]: ./P5/Submit/examples/RGB.png "RGB color space"
[image3]: ./P5/Submit/examples/HSV.png "HSV color space"
[image4]: ./P5/Submit/examples/HLS.png "HLS color space"
[image5]: ./P5/Submit/examples/YCrCb.png "YCrCb color space"
[image6]: ./P5/Submit/examples/HogFeature.png "Hog feature on car"
[image7]: ./P5/Submit/examples/HogFeature2.png "Hog feature on not car"
[image8]: ./P5/Submit/examples/SlidingWindows.png "Sliding window 128x128"
[image9]: ./P5/Submit/examples/SlidingWindows2.png "Sliding window 64x64"
[image10]: ./P5/Submit/examples/Search_S1.png "Search S1"
...
The final vehicle detection pipeline is combined with lane finding pipeline from an earlier project. We provide the final video [with](./output_videos/project_video_out_with_lane_diag.mp4) and [without](./output_videos/project_video_out_with_lane.mp4) additional diagnostic data.
...
Our cars are black and white and hence we are interested to see which color spaces clusters black and white pixels close together. From the scatter plots above, it looks like HLS color space has both black and white pixels on the end of the G axis. However, this visual inspection of pixels is rather subjective. We defer judgment on color space till we measure our model's prediction performance later.

#### 2.2 Histogram of Gradients

Histogram of Gradients was made popular by Dalal, Triggs[1]. The idea is that we partition an image into subregions and run multiple orientation kernels over the pixels. Similar to Sobel kernel, just over different directions. This allows us to obtain gradient strength in various orientations. Intuitively, HOG features captures the shape of objects. We find that visualization is helpful in gaining an understanding of hog extraction.
...
The best way to find out for sure is to fiddle with them and see what happens. The table below shows the results of our experimentation on 15 different combinations of techniques and parameters. The approach we took to obtain the most information without enumerating all possible combinations is to tweak only one aspect at a time, and build up our model once we ascertain the best choice of a particular aspect. The aspects we tested are color spaces, hog pixel per cell, hog cells per block, color histogram bin number, and spatial bin image size. The table below is most useful when looking within their respective section (*in italics*) as it tells us what aspect improves accuracy. See `ExtractAndTrainSVC` function in utils.py
...
Below are some examples our of vehicle detection using the modified sliding-window search and the SVM classifier we trained earlier. See `P5PipelineSimple` function in utils.py

![alt][image10]

#### 5. Improving detection robustness

Although we see from the example image above that our classifier correctly detects all instances of vehicle with zero false positive, there might be issues when we run our pipeline over other images. Thus we implemented several further enhancements to our pipeline. See `P5PipelineRobust` function in utils.py

#### 5.1 Two scale sliding-window

We added one more set of sliding-windows with a different scale. By using two sets of sliding-windows with different scales, we allowed our pipeline to be more capable in predicting vehicles of different sizes.
...
The pipeline may fail to detect vehicles under poor lighting or heavy rain which adds noise to the video frames, since these conditions were not specifically trained for. Another concern is that the classifier is trained on rather low resolution images of (64, 64) pixels and relatively low sample count (of about 9000 positive and 9000 negative images).
